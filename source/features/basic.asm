...
do_files:
	mov ax, disk_buffer			; get a copy of the filelist
...
.is_ok:
	...
	mov di, string_buffer			; Add ':' char to end for searching
	add di, ax
	mov al, ':'
	stosb
	mov al, 0
...
textmode_finish:
	...
	err_memory		db "Error: out of memory", 0
	err_nest_limit		db "Error: FOR or GOSUB nest limit exceeded", 0
	err_next		db "Error: NEXT without FOR", 0
	err_no_endif		db "Error: BLOCKIF without matching ENDIF", 0
	err_loop		db "Error: LOOP without DO", 0
	err_print_type		db "Error: PRINT not followed by quoted text or variable", 0
	err_polygon_min		db "Error: a polygon must have at least three points", 0
	err_polygon_max		db "Error: a polygon cannot have more than ten points", 0
...
; DATA SECTION
	...
	token			times 127 db 0	; Storage space for the token
...
vars_loc:
	...
	colourinfo_keyword	db "COLOURINFO", 0
